...
class LocalDataManager(DataManager):
    ...
    def create_chit(self, metadata: Dict[str, Any]) -> str:
        ...
        new_chit_df = pd.DataFrame([chit_record])
        if chits_df.empty:
            chits_df = new_chit_df
        else:
            chits_df = pd.concat([chits_df, new_chit_df], ignore_index=True)
        ...
        new_installments_df = pd.DataFrame(installment_records)
        if installments_df.empty:
            installments_df = new_installments_df
        else:
